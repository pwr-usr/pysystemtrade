...
> **Archive provenance**
>
> Source: [official XTQuant complete examples](https://dict.thinktrader.net/nativeApi/code_examples.html)  
> Retrieved: `2026-08-09T17:22:27Z`  
> Transformation: the VuePress article body was converted to GFM; site navigation, scripts, tab scaffolding, line-number markup, and UI chrome were removed. Only content present in the server-rendered article was retained.  
> **Coverage warning:** these examples are predominantly securities-oriented and are not an exhaustive or end-to-end Chinese-futures execution, recovery, or reconciliation guide.
...
while 1:
    ...
    for mid, v in ret_sector_dict.items():
        if v == '盘口价差':
            # 首次获取内容
            if mid not in sector_gt_dict:
                basket_list = api.pull_block(mid)
                sector_gt_dict[mid] = basket_list
                print(f'首次获取:{v} {mid} 长度:{len(basket_list)} 内容:{sector_gt_dict[mid]}')
            else:
                try:
                    # 监听因子变动, 如果有变化, 返回True 获取内容
                    if api.check_outdated(mid):
                        ...
                        print(f'因子:{v} 有新的更新!')
                        sector_gt_dict[mid] = basket_list
                        print(f'更新获取:{v} {mid} 长度:{len(basket_list)} 内容:{sector_gt_dict[mid]}')
                except:
                    pass
    time.sleep(5)
